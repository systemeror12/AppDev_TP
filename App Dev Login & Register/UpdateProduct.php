<?php
	...
	$prdsize=$row['Size']; 
	$prdprice=$row['Product_Price']; 

	 if(isset($_POST['submit'])){
		
        $prdnme =  $_POST['Prdname'];
        $prdsize = $_POST['PrdSize'];
        $prdprice =  $_POST['PrdPirce'];

		$sql = "UPDATE `tb_products` SET `Product_Id`='$id',
		`Product_Name`='$prdnme',`Size`='$prdsize',
		`Product_Price`='$prdprice' WHERE Product_Id=$id";
		$result = mysqli_query($conn, $sql);
		if($result){
		...
		<div class="forms">
			...
			<div class="form Register">
				...
				<form action="" method="post">
					...
					<div class="input-field">
					<input type="Productsize" name="PrdSize" placeholder="Product Size" value= "<?php echo $prdsize;?>" required>
					<i class="material-symbols-outlined">aspect_ratio</i>
					</div>

					<div class="input-field">
					<input type="text" name="PrdPirce" placeholder="Product Price" value= "<?php echo $prdprice;?>" required>
